...
elseif ($_REQUEST['act'] == 'info')
{
    ...
    $photo_users_files = $db->getAll($sql);
    if($photo_users_files){
        $photo_users_files = $photo_users_files[0];
...
elseif ($_REQUEST['act'] == 'delivery_ship')
{
    ...
    if ($order_finish)
    {
		...
		/*调用发短信通接口  Edison Add on 2017-05-30*/
        $deliver_number = $delivery['invoice_no']; //快递单号
        $delivery_company_name = $_REQUEST['delivery_company_name']; //快递公司名称
        $ch = curl_init();
        $url = "http://xingsom.com/data/index.php/Home/Alidayu/notifyBuyer";
        curl_setopt($ch, CURLOPT_URL, $url);
        $post_data = array("mobile" =>$order['mobile'],"deliver_number" =>$deliver_number,"company_name"=>$delivery_company_name);
