...
elseif ($_REQUEST['step']== 'ajax_update_cart')
{
    ...
    if (isset($_POST['rec_id']) &&isset($_POST['goods_number']))
    {
        ...
        foreach ($cart_goods['goods_list'] as $goods)
        {
         ...
         $market_price_desc = sprintf($cart_goods['total']['market_price']);
		 $save_rate = sprintf($cart_goods['total']['saving'], $cart_goods['total']['save_rate']);
        ...
        {
           $result['marke_total'] .= $market_price_desc;
		   $result['save_rate'] .=  $save_rate;
		   
        }
		
		$result['toal_all'] = '<p style="text-align: left; font-size:14px; font-weight:bold;">
						商品总金额：
						<em class="end">'.$cart_goods['total']['goods_price'].'</em>
					</p>';
