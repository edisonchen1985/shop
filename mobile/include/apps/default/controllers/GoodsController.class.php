...
class GoodsController extends CommonController {
	...
	if(!$openid){
		...
		$url = 'http://xingsom.com/data/index.php/Home/Check/checkOpenId/openid/'.$openid;
		$curl = curl_init();
    ...
    public function index() {
    	//获取用户是否在3D扫描仪上扫描过数据
	$user_data = $this->getUserSize();
	if($user_data == false){
            $this->assign('user_recommendation', 0);
	}else{
            $this->assign('user_recommendation', $user_data.'码');
	}
        // 获得商品的信息
