...
{
    $time = gmtime();
    $sql = 'SELECT g.*, c.measure_unit, b.brand_id, g.comments_number, g.sales_volume,b.brand_name AS goods_brand, m.type_money AS bonus_money, ' .
...
//获取二级分类下的商品
	...
	//获取三级分类下的商品
		...
		$cat_list_arr2  = cat_list($cat_id, 0 , false);

		foreach($cat_list_arr2 as $key=>$value)
		{
			...
			if($value['level'] == 2)
			{
				...
				if ($num > 0)
				{
				 ...
				 $cat_list_arr2[$key]['goods'] = $goods_res;
			}
			else
			{
				unset($cat_list_arr2[$key]);
			}
		}

	
	
	$cat['goods_level3'] = $cat_list_arr2;
...
function group_buy_info($group_buy_id, $current_num = 0)
{
    ...
    $sql = "SELECT b.goods_id,b.*,g.*, b.act_id AS group_buy_id, b.act_desc AS group_buy_desc, b.start_time AS start_date, b.end_time AS end_date " .
